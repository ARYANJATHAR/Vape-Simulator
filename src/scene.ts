...
import { VapeSimulation } from './simulation';
    ...
    start() {
        ...
        const draw = (time: number) => {
            ...
            if (this.cameraActive && this.video.readyState >= 2 && this.video.videoWidth > 0 && this.video.videoHeight > 0) {
                ...
                bg.addColorStop(0, '#292e38');
                bg.addColorStop(.65, '#15191f');
            ...
            if (this.demoActive && this.demoMouth) {
                ...
                ctx.setLineDash([]); ctx.fillStyle = '#ffffe9'; ctx.font = '12px Space Grotesk, sans-serif'; ctx.textAlign = 'center';
            ...
            const face = this.interaction.face;
            const nearMouth = face && Math.hypot(result.tip.x - face.mouth.x, result.tip.y - face.mouth.y) < Math.max(26, face.width * .38);
            const powered = this.active && !this.paused && result.held && !result.trackingLost && (result.atMouth || nearMouth);
            this.drawDevice(ctx, result.pose, powered ? 1 : 0);
            this.vapor.draw(ctx);
